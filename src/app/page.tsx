...
export default function Home() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-8 space-y-6">
      ...
      {/* Progress — rotating messages (CSS-only animation) */}
      {(loading || parsing) && (
      ...
      {!result && !loading && !parsing && (
        <section className="space-y-4">
          ...
          />
          <button
            onClick={analyze} disabled={!ready}
            className={`w-full py-3 rounded-lg text-sm font-medium transition cursor-pointer active:scale-[0.98] ${ready ? "bg-accent text-white hover:bg-accent-dim" : "bg-ink-100 text-ink-300 cursor-not-allowed"}`}
          >
            {ui.btnAnalyze}
          </button>
      ...
      {result && (
        <div ref={resultsRef} className="space-y-6">
          {/* Score — ghost style, discrete */}
          <div className="text-center py-4">
            <span className="font-[family-name:var(--font-mono)] text-[56px] font-light text-ink-200 leading-none">{result.score.total}</span>
            <span className="font-[family-name:var(--font-mono)] text-sm text-ink-300 ml-1">/100</span>
            <p className="font-[family-name:var(--font-mono)] text-[11px] text-ink-300 tracking-widest uppercase mt-1">{ui.scoreMeta}</p>
          </div>
          <p className="text-sm text-ink-600 leading-relaxed">{result.score.summary}</p>

          {/* Strengths — closed by default */}
          ...
          {/* Improvements — open by default (hero of the page) */}
          ...
          <details open className="border border-accent/30 rounded-lg">
            <summary className="px-4 py-3 text-sm font-medium text-accent">{ui.improvedCvTitle}</summary>
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="border border-ink-100 rounded-lg p-4">{renderResume(result.improved_cv.text)}</div>
      ...
      <footer className="pt-12 pb-6 text-center space-y-2">
        <div className="flex items-center justify-center gap-4 text-xs text-ink-400">
          <span className="font-[family-name:var(--font-geist)] font-medium"><span className="text-ink-700">cv</span><span className="text-accent">ool</span></span>
          ...
          <a href="https://buymeacoffee.com/alfredoarenas" target="_blank" rel="noopener noreferrer" className="hover:text-ink-600 transition">{ui.donationBtn}</a>
        </div>
        <p className="text-[11px] text-ink-300">{ui.powered}</p>
      </footer>
    </div>
  );
}
